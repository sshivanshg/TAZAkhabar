...
public static class LocalPublisherSourceCatalog
{
    ...
    /// <summary>Google News RSS scoped to one publisher domain plus a city/region query.</summary>
    public static string GoogleNewsSiteRss(string host, string query) =>
        $"https://news.google.com/rss/search?q={Uri.EscapeDataString($"site:{host} {query} when:7d")}&hl=en-IN&gl=IN&ceid=IN:en";
