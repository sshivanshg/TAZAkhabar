...
try
{
    ...
    if (builder.Environment.IsDevelopment())
    {
        builder.Services.PostConfigure<UploadOptions>(upload =>
        {
            ...
            {
                upload.RootPath = Path.Combine(Path.GetTempPath(), "khabro-uploads");
            }
        });
    }
    ...
    builder.Services.AddHttpClient("rss", client =>
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("KhabroIngest/0.1");
    });
    ...
    {
        client.Timeout = TimeSpan.FromSeconds(15);
        client.DefaultRequestHeaders.UserAgent.ParseAdd("KhabroNotifications/0.1");
    });
    ...
    {
        options.SwaggerDoc("v1", new() { Title = "Khabro API", Version = "v1" });
    });
