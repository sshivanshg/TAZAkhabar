#!/usr/bin/env python3
"""Persist Codex agent sessions under .codex/sessions/ (gitignored).

Handles sessionStart, sessionEnd, and stop events from hooks.json.
Copies the conversation transcript when Codex provides transcript_path.
"""
...
def handle_session_start(payload: dict, root: Path) -> dict:
    ...
    return {
        ...
        "additional_context": (
            ...
            f".codex/sessions/{session_id}/ (local only, gitignored)."
        ),
    }
